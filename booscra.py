...
def extract_book_links(doc):
    ...
    return book_links
extract_book_links() #renverra un tableau d'URL pour les pages.
...
def csv_home_pag(book_links):
    ...
    csv_home_pag(extract_book_links(doc)).head()
    print(doc)
    section = doc.find('li',{'class':'next'}) next_url = section.contents[0]['href'] base_url = "http://books.toscrape.com/" a = base_url + next_url

                    response = requests.get(a)

                    page_contents = response.text
    doc1 = BeautifulSoup(page_contents, 'html.parser')
    response = requests.get(a) page_contents = response.text
    doc1 = BeautifulSoup(page_contents, 'html.parser')
